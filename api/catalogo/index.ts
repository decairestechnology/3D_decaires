...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === 'POST') {
    const { codigo, nome, material_id, peso_padrao_g, tempo_impressao_h, preco_padrao, descricao } = req.body
    try {
      const [novo] = await sql`
        INSERT INTO catalogo_produtos (codigo, nome, material_id, peso_padrao_g, tempo_impressao_h, preco_padrao, descricao)
        VALUES (${codigo}, ${nome}, ${material_id ?? null}, ${peso_padrao_g ?? null}, ${tempo_impressao_h ?? null}, ${preco_padrao ?? null}, ${descricao ?? null})
        RETURNING *
      `
  ...
  if (req.method === 'PATCH' && id) {
    const { codigo, nome, material_id, peso_padrao_g, tempo_impressao_h, preco_padrao, descricao, ativo } = req.body
